...
Fecha inicial: 6 de septiembre de 2026. Actualizado el 7 de septiembre con decisiones de Esteban sobre validaciones y sesión y con el primer incremento de registro de propietarios. Este documento recoge decisiones y estado de implementación; la especificación OpenAPI se genera desde el código. Los apartados de login, sesión, edición de perfil y foto siguen distinguiendo sus propuestas y pendientes.
...
- Arquitectura prevista: Spring Boot, PostgreSQL, monolito MVC, JWT, roles y permisos. JWT y autorización por permisos siguen pendientes de implementación.
...
Para el contrato de acceso se utilizan `email` como cadena con formato de correo y `password` como cadena; ambos obligatorios. La contraseña es información de entrada y no se devuelve en las respuestas. Esteban solicita login correcto con `200 OK` y JWT de 24 horas sin tokens de renovación. La forma concreta de la respuesta y los controles de sesión se proponen en «Concreciones del 7 de septiembre».
...
El objetivo confirmado es conducir a registrar la primera mascota. En la respuesta de seguimiento Esteban priorizó la alternativa más sencilla de implementar. Se adopta **registro → login → primera mascota**. El primer paso está implementado: `POST /api/v1/auth/registrations` devuelve `201 Created` con `id` y `email`, sin token ni redirección HTTP. El frontend deberá mostrar el éxito y conducir a `/login`; la autenticación y emisión de JWT se desarrollarán en el incremento de login. El registro con inicio automático sería viable reutilizando el mismo emisor, pero añadiría ese comportamiento al alta. La creación de una mascota será una operación posterior de P04; el éxito del alta no depende de ella.
...
Confirmado por Esteban: login correcto `200 OK`, JWT con identificación, correo, rol y permisos, duración de un día y ausencia de tokens de renovación. Propuesta precisa: 24 horas desde su emisión, `exp = iat + 86400` segundos. Al vencer es necesario volver a autenticarse.

Propuesta de respuesta `UserLoginResponseDTO`: `accessToken` como cadena, `tokenType` con valor `Bearer`, `expiresIn` como entero en segundos y `user` como objeto con `id`, `email`, `role` y `permissions`. Esta forma está por concretar en OpenAPI y no es un DTO implementado.

Propuesta de claims: `sub` contiene el UUID de la cuenta (cumple la función del `id` solicitado), `email`, `role`, `permissions`, `iat`, `exp`, `jti`, `iss` y `aud`. Evitar duplicar `sub` e `id` dentro del JWT. `iat`/`exp` son segundos Unix; `jti` identifica el token, `iss` al emisor y `aud` al destinatario. Un JWT firmado se puede leer: no incluir contraseña, hash, teléfono, nacimiento ni foto. [RFC 7519](https://www.rfc-editor.org/rfc/rfc7519.html).

Los permisos representan acciones y se generan en el servidor a partir de un catálogo fijo por rol; sus nombres finales saldrán de las operaciones acordadas. Un permiso para editar perfil no concede acceso al perfil de otra persona. El backend comprueba también la propiedad o relación con el recurso. Para evitar permisos antiguos durante 24 horas, se propone consultar `active` y el rol vigente en cada petición autenticada y construir las autoridades desde esa información. Los claims orientan la interfaz como fotografía del momento del login. [OWASP: autorización en cada petición](https://cheatsheetseries.owasp.org/cheatsheets/Authorization_Cheat_Sheet.html#validate-the-permissions-on-every-request).
...
La política de 24 horas requiere una prueba del límite de `exp` y de la tolerancia de reloj configurada; no prometer un vencimiento estricto si el validador admite segundos adicionales. La implementación utilizará soporte de Spring Security con algoritmo permitido y comprobaciones de firma, emisor, destinatario y tiempo. [Spring Security: validación JWT](https://docs.spring.io/spring-security/reference/servlet/oauth2/resource-server/jwt.html#oauth2resourceserver-jwt-validation).

Propuesta de errores: token ausente, inválido, vencido o revocado devuelve `401` en JSON. El frontend centraliza su tratamiento, limpia su estado y navega a `/login`, como solicita Esteban, con un mensaje comprensible. Un usuario autenticado sin permiso recibe `403` y una pantalla de acceso denegado. La API no devuelve HTML de login ni depende de una redirección HTTP para autorizar. [OWASP: estados HTTP para API](https://cheatsheetseries.owasp.org/cheatsheets/REST_Security_Cheat_Sheet.html#http-return-code).
...
El nombre quedó definido e implementado con el límite de 150 y dos o más componentes. Permanecen por definir el seguimiento del paso de primera mascota, el catálogo de permisos de estas operaciones y el acuerdo de almacenamiento del token con los frontends; los límites de foto y detalles de revocación se mantienen como propuestas técnicas. Cada decisión se resuelve antes de implementar el comportamiento que dependa de ella. Estos pendientes no impiden preparar incrementos cuyo alcance ya esté definido.

El registro aporta implementación a SCRUM-23 y contrato generado a SCRUM-67; la evidencia debe especificar ese alcance parcial. El vínculo existente SCRUM-67 → SCRUM-6 debe reflejar explícitamente este trabajo incremental, sin dar por terminada la revisión del contrato ni eliminar la dependencia silenciosamente. La publicación versionada del alcance completo y revisión de los tres siguen siendo requisitos de cierre de SCRUM-67. La implementación conserva V1 y añade V2; login, JWT, edición y foto siguen pendientes.
...
2. Generar la especificación mediante springdoc. Sus salidas configuradas son `/v3/api-docs` (JSON) y `/v3/api-docs.yaml` (YAML); Swagger UI permite visualizarlas en `/swagger-ui/index.html`. La seguridad del primer incremento permite esas rutas y el registro; la disponibilidad efectiva se comprueba al verificar la aplicación.
...
`ApiSecurityErrorHandler` adapta también `401` y `403` desde los filtros, que pueden fallar antes del controlador. Ambos manejadores comparten una fábrica de errores y un `instance` aleatorio `urn:uuid:...`. No se incluyen contraseñas, hashes, valores rechazados, SQL ni mensajes internos de excepciones. El log de un `500` en el handler usa identificador, clase y ubicaciones de código, sin copiar el mensaje potencialmente sensible. Esto prepara errores uniformes; todavía no implementa JWT ni permisos funcionales.
...
## Alcance del primer OpenAPI y siguientes incrementos

La primera operación elegida ya tiene código, DTOs y anotaciones: `POST /api/v1/auth/registrations`, con respuesta `201` que contiene `id` y `email`. La especificación generada debe verificarse y compartirse con ese alcance parcial. El siguiente incremento puede incorporar login, usando las decisiones de `200 OK`, JWT de 24 horas y ausencia de renovación. El paso pendiente de mascota, permisos, operaciones de foto y almacenamiento del token se concretan antes de sus respectivos incrementos. Cada operación ampliará la misma especificación desde el código; no hace falta escribir un YAML manual completo para avanzar.
